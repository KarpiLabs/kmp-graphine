...
class GraphState<T>(
    ...
    suspend fun fitToScreenAnimated(
        ...
        if (_nodeStates.isEmpty() || !viewportWidth.isFiniteNumber() || !viewportHeight.isFiniteNumber() || viewportWidth <= 0f || viewportHeight <= 0f) return
    ...
    fun setNodePositions(positions: Map<String, Offset>) {
        positions.forEach { (id, pos) ->
            // Security guard: Ignore non-finite position updates to prevent node state corruption
